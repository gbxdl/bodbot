...
class theSuperComputer:
    ...
    def updateBestDataSeen(self, rider_info: RiderInfo | None):
        ...
        self.bestThisYear = max(self.bestThisYear, self.getPointsThisYear(rider_info))
        self.bestLastYear = max(self.bestLastYear, self.getPointsLastYear(rider_info))
    ...
    def determineComment(self, rider_info: RiderInfo | None):
        ...
        if (self.getPointsThisYear(rider_info) > self.bestThisYear):
            return "die dit jaar goed is (thisYear:" + str(self.getPointsThisYear(rider_info)) + ")"
        if (self.getPointsLastYear(rider_info) > self.bestLastYear):
            return "die vorig jaar goed was (lastYear:" + str(self.getPointsLastYear(rider_info)) + ")"
        return "die niks kan"
    ...
    def getPointsThisYear(self, rider_info):
    ...
    def applyInflation(self, multiple):
        ...
        self.bestThisYear *= multiple
        self.bestLastYear *= multiple
    ...
    def updateStat(self, statName, statVal):
        match statName:
            ...
            case "lastYear":
                self.bestLastYear = max(self.bestLastYear, int(statVal) * self.inflationActive)
            case "thisYear":
                self.bestThisYear = max(self.bestThisYear, int(statVal) * self.inflationActive)
